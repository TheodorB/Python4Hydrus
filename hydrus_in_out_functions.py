...
# %%    
def change_par_selectorin(run_path,par,par_new_value):
    '''
...
#     run_path = os.path.join(path,example)
    ...
    text[row+1][text[row].index(par)] = str(par_new_value)
...
#%%
def get_par_selectorin(run_path,par):
    '''
...
#     run_path = os.path.join(path,example)
    ...
    # find variable
    row =  [key for key, value in text.items() if par in value][0]
    return  text[row+1][text[row].index(par)] # return desired parameter in Selector in file 
...
#     df[['1','Mat', 'Lay']] = df[['1','Mat', 'Lay']].astype(str) # these columns should be integer strings
    ...
    end_row   = start_row + n_times - 1
#    num_pars = len(mytext[start_row])
#    mypars = mytext[start_row-1][:] # parameters for column names  of dataframe     
...
#     print start_of_last_part
#     range_last_keys = range(start_of_last_part, mytext.keys()[-1]+1)
